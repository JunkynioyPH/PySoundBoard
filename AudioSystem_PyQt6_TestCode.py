...
from PyQt6.QtWidgets import QApplication, QPushButton, QMainWindow, QVBoxLayout, QWidget, QLabel
import sys
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        poolText = QLabel()
        VBox.addWidget(poolText)
        def poolDisplayFunc():
            poolText.setText(self.sound.status(cli=False))
        self.poolDisplay = QTimer(self)
        self.poolDisplay.timeout.connect(poolDisplayFunc)
        self.poolDisplay.start(250)
        ...
        VBox.addWidget(FuncButton(f'MultiModeA', lambda: self.sound.toggleState('audio','multi')))
        VBox.addWidget(FuncButton(f'MultiModeS', lambda: self.sound.toggleState('sound','multi')))
        VBox.addWidget(FuncButton(f'LoopingA', lambda: self.sound.toggleState('audio','loop')))
        VBox.addWidget(FuncButton(f'LoopingS', lambda: self.sound.toggleState('sound','loop')))
